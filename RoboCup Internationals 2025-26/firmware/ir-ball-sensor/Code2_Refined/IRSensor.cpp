...
// ---- calibration storage (defaults; overwrite from the Calibration sketch) -
uint16_t IR_baseline[NUM_SENSORS];
float    IR_gain[NUM_SENSORS];

// Default distance LUT - REPLACE with YOUR measured values (README step 5).
// peak intensity (10-bit, = baseline - raw) -> distance cm. Higher = closer.
...
void IRSensor::begin()
{
  for (uint8_t i = 0; i < NUM_SENSORS; i++) {
    ...
    IR_baseline[i] = ADC_MAX;    // safe default until calibrated
    IR_gain[i]     = 1.0f;
  }
