...
All microcontroller code for the robot. Each sub-folder targets one board. Sketch
folders keep their exact names because the Arduino IDE requires the folder name to
match the `.ino`, and most folders also carry a `platformio.ini` for PlatformIO.
...
| `ir-ball-sensor/Code2_Calibrated` | IR ring (Teensy 4.1) | ✅ flight build | calibration baked in (2026-06-27) |
| `ir-ball-sensor/Code2_Refined` | IR ring | reference | same code, uncalibrated defaults |
| `ir-ball-sensor/Calibration` | IR ring | tool | baseline capture + Serial-Plotter readout |
...
| `line-sensor/line_pcb_baseline` | Line ring (Teensy 4.1) | ✅ auto-learn detector | 1 s green baseline, then drop-by-5 |
| `line-sensor/line_pcb_detect` | Line ring | ✅ fixed-threshold detector | per-sensor thresholds + hysteresis |
| `line-sensor/line_pcb_raw_reader` | Line ring | tool | raw ADC streamer for calibration |
...
| `motor-current/motor_current_supervisor` | Nano Every (optional) | watchdog | stall-timer over-current protection |
| `motor-current/motor_current_limit_1A` | Nano Every (optional) | watchdog | simple per-motor 1 A chop |
